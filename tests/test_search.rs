...
mod tests {
    ...
    use pks::search::retriever::{SearchBackend, SearchResult, TantivyBackend, ChunkMeta};
    ...
    fn hybrid_search_with_vectors_outperforms_bm25_only() {
        ...
        let hybrid_results = search_hybrid(&backend.vectors, &query_vector, bm25_results, 5, &backend.chunk_meta);
    ...
    fn vector_only_hit_carries_correct_metadata() {
        ...
        backend.add_chunk_with_vector(&chunk_a, vector_a).unwrap();
        // Deliberately skip commit so BM25 index has no documents; chunk_meta is populated.
        backend.commit().unwrap();
        ...
        let results = search_hybrid(&backend.vectors, &query_vector, bm25_results, 5, &backend.chunk_meta);

        assert!(!results.is_empty(), "vector-only hit must appear in hybrid results");
        let top = &results[0];
        assert_eq!(top.file_path, "vec_only.md", "file_path must be recovered from chunk_meta");
        assert_eq!(top.repo_id, "repo_vec", "repo_id must be recovered from chunk_meta");
