...
impl ShadowJournalHook {
    ...
    pub(crate) fn render_journal_md(&self) -> String {
        ...
        let duration_mins = Utc::now()
            .signed_duration_since(self.started_at)
            .num_minutes();
        let mut tool_counts: HashMap<&str, usize> = HashMap::new();
        for e in &self.entries {
            *tool_counts.entry(e.tool_name.as_str()).or_default() += 1;
        }
        let mut tools_sorted: Vec<(&str, usize)> = tool_counts.into_iter().collect();
        tools_sorted.sort_by(|a, b| b.1.cmp(&a.1));
        let tools_summary = tools_sorted
            .iter()
            .map(|(n, c)| format!("{n} ({c})"))
            .collect::<Vec<_>>()
            .join(", ");
        ...
        for e in &self.entries {
            for fp in &e.file_paths {
                if seen.insert(fp.clone()) {
                    files.push((fp.as_str(), e.tool_name.as_str(), e.tool_input_summary.as_str()));
                }
            }
        }
        ...
        if self.dropped_events > 0 {
            md.push_str(&format!("\n_{} events dropped (max_entries reached)._\n", self.dropped_events));
        }
        md
    }
}
